...
- `rebalancing.market_cap_comparison(df, market_weights, by_retirement=False)` expects an already look-through-decomposed frame (same as `analysis.concentration_breakdown` gets on the decomposition path); it filters to `equities` and renormalizes internally. Tiers are `large_cap` / `mid_cap` / `small_cap` / `emerging` (emerging markets is its own tier regardless of cap); value that doesn't resolve to a tier is redistributed by the benchmark's developed-market tier ratios. `by_retirement=True` splits the equity sleeve into retirement/non-retirement halves (adds an `is_retirement` column, each half sums to 100) — it silently no-ops if the frame lacks `is_retirement`.
