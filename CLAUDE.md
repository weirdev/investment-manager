...
- `rebalancing` compares the equity sleeve to the global benchmark on two axes: `market_cap_comparison` (4 tiers: `large_cap` / `mid_cap` / `small_cap` / `emerging`) and `region_comparison` (the 7-cell `us`/`ex_us` × large/mid/small grid, plus one `emerging` cell). Both take an already look-through-decomposed frame (same as `analysis.concentration_breakdown` gets on the decomposition path), filter to `equities`, and renormalize internally so `portfolio_pct` / `market_pct` each sum to 100. Value that doesn't resolve to a bucket is redistributed by the benchmark's own ratios; `region == "emerging"` value always lands in the emerging bucket. `by_retirement=True` splits the sleeve into retirement/non-retirement halves (adds a leading `is_retirement` column, each half sums to 100) — silently no-ops if the frame lacks `is_retirement`. `/api/rebalancing` returns `{market_cap, regions, total, equity_total}`.
